...
function Navbar() {
  ...
  return (
    ...
    >
      <div
        id="smallNav"
        className="flex sm:hidden h-[56px] from-[#0C2263] via-[#300C4D] to-[#3A0647] "
      >
        <div className="flex w-full justify-between items-center px-4">
          <h1 className="text-[1.1rem] text-white">FOG SERVER</h1>
          <button
            onClick={handleNavClick}
            className="border border-gray-600 text-gray-300 bg-[rgba(255,255,255,0.1)] p-2 rounded "
          >
            <svg
      ...
      <Link href={"/dashboard"} className="">
        ...
        <Link href={"/"} className="">
          <div className=" h-12 rounded flex items-center hover:bg-[rgba(255,255,255,0.25)] px-4 py-2">
            <Icon height={"20px"} className="mx-1" icon="mingcute:device-fill" />
            <p>Device List</p>
          </div>
        </Link>
